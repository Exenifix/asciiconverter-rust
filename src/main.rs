...
fn process_image_grayscale(path: &str, resolution: f32) -> Result<String, Box<dyn std::error::Error>> {
...
fn process_image_rgb(path: &str, resolution: f32) -> Result<String, Box<dyn std::error::Error>> {
    ...
    let max_brightness = resized_image.pixels().map(|p| rgb_to_brightness(p[0], p[1], p[2])).max().unwrap_or(1) as f32;
    ...
    for y in 0..new_h {
        for x in 0..new_w {
            ...
            output.push(brightness_to_char((rgb_to_brightness(r, g, b) as f32 / max_brightness * 255.) as u8));
        }
        output.push('\n');
    }
